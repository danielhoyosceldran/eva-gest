...
public partial class VendaDialogViewModel : DialegViewModelBase
{
    ...
    private VendaDialogViewModel(IVendaService vendes, ISoundService so)
    {
        _vendes = vendes;
        _so = so;
    }
    ...
    public static async Task<VendaDialogViewModel> Nova(
        ...
        ITreballadoraService treballadores, ISoundService so)
    {
        var vm = new VendaDialogViewModel(vendes, so) { ModeActual = Mode.Nova };
        await vm.CarregarOpcions(clients, cataleg, treballadores);
        return vm;
    }

    /// <summary>Prefilled from a completed appointment (RF-09).</summary>
    public static async Task<VendaDialogViewModel> DesDeCita(
        IVendaService vendes, IClientService clients, ICatalegService cataleg,
        ITreballadoraService treballadores, ISoundService so, int citaId)
    {
        var vm = new VendaDialogViewModel(vendes, so) { ModeActual = Mode.DesDeCita };
        await vm.CarregarOpcions(clients, cataleg, treballadores);

        var venda = await vendes.PreparaDesDeCita(citaId);
        vm._citaId = citaId;
    ...
    public static async Task<VendaDialogViewModel> Editar(
        IVendaService vendes, IClientService clients, ICatalegService cataleg,
        ITreballadoraService treballadores, ISoundService so, Venda venda)
    {
        var vm = new VendaDialogViewModel(vendes, so) { ModeActual = Mode.Editar };
        await vm.CarregarOpcions(clients, cataleg, treballadores);
    ...
    private async Task CarregarOpcions(IClientService clients, ICatalegService cataleg, ITreballadoraService treballadores)
    {
    ...
    private void RecalcularTotals()
    {
        ...
        var d = IvaCalculator.Calcular(linies, IvaMode.Inclos);
        BaseCents = d.BaseCents;
        IvaCents = d.IvaCents;
        TotalCents = d.TotalCents;
        Desglossament = IvaCalculator.CalcularPerTipus(linies, IvaMode.Inclos);
